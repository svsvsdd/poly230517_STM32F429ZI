...
void printToBuffer(const char* format, ...)
{
    ...
    buffer_index += vsnprintf(print_buffer + buffer_index, BUFFER_SIZE - buffer_index, format, args);
    va_end(args);
}

int _write(int file, char *ptr, int len)	//문자?�� 주소�? 받음 length?��
  {
  	HAL_UART_Transmit(&huart3, (uint8_t *)ptr, len, 500);	//문자?��?�� 보냄

  	return len;
  }

  int __io_putchar(int ch)	// 문자 ?��?�� 보냄
  {
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
//	  int prev_cnt = -1; // initialize prev_cnt to an impossible value
//	  int new_cnt = -1;
	  	  if (cnt == 0){
			  number0();
...
//		  new_cnt = cnt;
//
//		  /* only print if there's a change in count value */
//		  if (new_cnt != prev_cnt)
//		  {
//			printf("%d\n\r", new_cnt);	//값이 달라질때만 출력되도록
//		  }
//		}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if(GPIO_Pin == GPIO_PIN_13 && (current_tick_1 - old_tick_1 > 100)) //pc13
	{	cnt++;
		if (cnt > 9) {
			cnt = 0;
		}
	...
	if(GPIO_Pin == GPIO_PIN_2 && (current_tick_2 - old_tick_2 > 100)) //pb2
	{	cnt--;
		if (cnt < 0) {
			cnt = 9;
		}
